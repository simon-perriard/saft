    ...
    pub(crate) fn parity_scale_codec_dispatch<'tcx>(
        ...
        match path {
            "parity_scale_codec::Encode::using_encoded" => {
                ...
                let summary_key = (*args_summary_keys.get(1).unwrap()).clone();
                let closure_cost = transfer_function.get_summary_for_key(&summary_key.unwrap());
...
pub(crate) mod sp_io_specs {
    use crate::analysis::{
        ...
        types::Type, cost_domain::CostDomain,
    };
        ...
        match path {
            "sp_io::hashing::blake2_256" => {
                if callee_info.args.is_empty() {
                    // Function is passed as a constant function and we don't know what will be hashed
                    // outter function must take care of this cost
                    let mut summaries = transfer_function.summaries.borrow_mut();
                    let summary_key = (callee_info.callee_def_id, vec![]);

                    let mut cost = CostDomain::new();
                    cost.add_steps(Cost::Infinity);

                    summaries.insert(summary_key, Some(cost));
                } else {
                    transfer_function.domain_state.add_steps(cost_to_big_o(
                        Type::from_mir_ty(
                            transfer_function.tcx,
        ...
        pub fn get_access_cost<'tcx>(
            ...
            match action_short {
                ...
                "get" => {
                    ...
                    transfer_function
                        .domain_state
                        .add_steps(cost_to_big_o(field.get_size(transfer_function.tcx)));
                    // storage access
                    transfer_function
                        .domain_state
                        .add_reads(field.get_size(transfer_function.tcx));
                ...
                "kill" => {
                    ...
                    transfer_function.domain_state.add_writes(Cost::Scalar(1));
                }
                "mutate" => {
