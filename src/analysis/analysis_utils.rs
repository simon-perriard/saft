...
pub(crate) mod def_id_printer {
    ...
    /// Returns the string representing the "def_id"'s name, stripped
    /// out of its path.
    /// # Arguments
    ///
    /// * `tcx` - Compilation context
    /// * `def_id` - DefId of interest
...
pub(crate) mod dispatchables_getter {
    ...
    /// Returns the HirIds of the extrinsics in the Call enum
    /// generated by the #[pallet::call] by analysis the HIR nodes
    /// # Arguments
    ///
    /// * `tcx` - Compilation context
    pub(crate) fn get_call_enum_variants_names(tcx: TyCtxt) -> Vec<&str> {
    ...
    pub(crate) fn get_dispatchable_def_ids(
        ...
        for match_target in variant_ids {
            let rustc_hir::Body { value, .. } = body;
            {
                ...
                if let Some((hir_id, qpath)) = called_fn_path {
                    ...
                    if let Some(def_id) = typeck_results.qpath_res(qpath, *hir_id).opt_def_id() {
                        ...
                        println!("function '{}' not found", match_target);
                    }
                } else {
                    println!("function '{}' not found", match_target);
                }
    ...
    // Recursively visits the "dispatch_bypass_filter" function's HIR
    // until it finds a the call to the "match_target" dispatchable
    // function name
