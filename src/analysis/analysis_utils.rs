...
/*pub(crate) mod cfg_helper {
    ...
    struct BasicBlockCallingCtxt {
        calling_ctxt: BitSet<BasicBlock>
    }

    trait LoopFinder<'tcx, 'preprocess> {
        fn visit_body(&mut self) {

        }

        fn visit_basic_block(&mut self, )
    }

    pub fn has_loop<'tcx, 'preprocess>(body: &'preprocess Body<'tcx>) -> bool {
